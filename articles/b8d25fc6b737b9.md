...
ダウンロードが終了したら、Linux 環境へ FTP などで仮想マシンへ格納して下さい。
...
```shell:bash
[root@dlp ~]#
  dnf -y install bc binutils elfutils-libelf elfutils-libelf-devel fontconfig-devel glibc glibc-devel ksh libaio libaio-devel libgcc libgcc.i686 libstdc++ libstdc++-devel libXi libXtst libnsl make sysstat gcc gcc-c++ libXrender libXrender-devel libX11 libXau libxcb smartmontools java-1.8.0-openjdk
```
...
```shell:bash
[oracle@dlp ~]#
  mv /u01/app/oracle/product/19.3.0/dbhome_1/jdk/jre /u01/app/oracle/product/19.3.0/dbhome_1/jdk/jre_org
  readlink -e /usr/bin/java /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.312.b07-1.el8.x86_64/jre
  cp -r /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.312.b07-1.el8.x86_64/jre /u01/app/oracle/product/19.3.0/dbhome_1/jdk/jre
  ll /u01/app/oracle/product/19.3.0/dbhome_1/jdk
    合計 5384
...
```shell:bash
[oracle@dlp ~]#
  vi /etc/sysconfig/oracle.oracledb
```
...
```shell:bash
[root@dlp ~]#
  vi /usr/lib/systemd/system/oracle@lsnrctl.service
```
...
```shell:bash
[root@dlp ~]#
  vi /usr/lib/systemd/system/oracle@oracledb.service
```
...
[root@dlp ~]#
  ...
  systemctl enable --now oracle@lsnrctl oracle@oracledb
```
