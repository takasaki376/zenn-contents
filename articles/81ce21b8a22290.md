...
- 可変長データを扱うのが得意
- 過去の情報を隠れ状態で保持しながら演算可能

## エンコーダ・デコーダモデル

- エンコーダネットワークとデコーダネットワークを用いて、入力データを出力データに変換
- RNN に基づくエンコーダ・デコーダモデルでは RNN エンコーダで入力系列を特徴表現に変換し、RNN デコーダで変換した特徴表現から出力系列を生成

## 注意機構

- デコード時に注目する入力要素を算出
- ドットアテンション：デコーダの隠れ状態とエンコーダの隠れ状態の内積に基づき注意スコアを算出
